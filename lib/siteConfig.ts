...
export async function getSiteConfig() {
  ...
  return {
    ...
    purchasePolicy: (config as any).purchasePolicy ?? "",
    cancellationPolicy: (config as any).cancellationPolicy ?? "",
    costoEnvio: (config as any).costoEnvio ?? 0,
    envioGratisDesde: (config as any).envioGratisDesde ?? 0,
